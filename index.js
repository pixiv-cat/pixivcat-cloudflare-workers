...
async function handleRequest(request) {
  ...
  if (checkRequest.is_valid === false) {
  ...
  } else if (checkRequest.is_manga === false) {
    // Normal mode
    ...
    const pixivApi = await _callPixivApi(
      `https://public-api-secure-pixiv-net.pixiv.cat/v1/works/${
        checkRequest.pixiv_id
      }.json?image_sizes=large`,
      token
    );

    if (pixivApi["status"] !== "success")
      return new Response("這個作品可能已被刪除，或無法取得。", {
        status: 404
      }); // Not found
    if (pixivApi["response"][0]["is_manga"])
      return new Response(
        "這個作品ID中有" +
          pixivApi["response"][0]["page_count"] +
          "張圖片，需要指定頁數才能正確顯示。",
        { status: 404 }
    ...
    image.headers.set(
      "X-Origin-URL",
      pixivApi["response"][0]["image_urls"].large
    );
    image.headers.set(
      "Content-Disposition",
      'inline; filename="' +
        _parseFilenameFromUrl(pixivApi["response"][0]["image_urls"].large) +
        '"'
    );
    image.headers.delete("Via");
    return image;
  } else if (checkRequest.is_manga === true) {
    // Manga mode
    if (checkRequest.pixiv_page < 0)
      return new Response("頁數不得為0。", { status: 404 }); // Specified page is 0.
    ...
    if (!pixivApi["response"][0]["is_manga"])
      return new Response(
        "這個作品ID中有只有一張圖片，不需要指定是第幾張圖片。",
        { status: 404 }
      ); // This Pixiv ID is Normal mode but the page is specified.
    if (
      checkRequest.pixiv_page + 1 > pixivApi["response"][0]["page_count"] ||
      checkRequest.pixiv_page < 0
    )
      return new Response(
        "這個作品ID中有" +
          pixivApi["response"][0]["page_count"] +
          "張圖片，您指定的頁數已超過這個作品ID中的頁數。",
        { status: 404 }
      ); // The specified page is more than total pages of this ID.

    let image = await _getImage(
      pixivApi["response"][0]["metadata"]["pages"][checkRequest.pixiv_page][
        "image_urls"
      ].large
    );
    image = new Response(image.body, image);
